...
def InitPopulation(Problem, InitialPopulation, Size, *args):
    ...
    if InitialPopulation and not isinstance(InitialPopulation, list):
        raise ValueError('Initial population must be defined in a list of dictionaries.')
    else:
        # Check for size
    ...
    for i in range(len(InitialPopulation), Size):
        Population.x[i, :] = np.array(Problem.LB[:Problem.Variables]) + \
                             (np.array(Problem.UB[:Problem.Variables]) - np.array(Problem.LB[:Problem.Variables])) * np.random.rand(Problem.Variables)
        Problem, Population.f[i] = ObjEval(Problem, Population.x[i, :], *args)
...
# print(rGA(myProblem, InitialPopulation=InitialPopulation, Options=None))
